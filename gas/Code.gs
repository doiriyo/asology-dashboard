// ============================================================
// asology インサイト自動集計 - Google Apps Script v3
// ============================================================
//
// 【概要】
...
function getSetting(key) {
  ...
  if (!sheet) {
    throw new Error('「設定」シートが見つかりません。先にメニューの「📊 asology → 初期セットアップ」を実行してください。');
  }
...
function notifyTokenError(errorMessage) {
  ...
  MailApp.sendEmail({
    to: email,
    subject: '【asology】アクセストークンの更新に失敗しました',
    body: `asologyインサイト自動集計のアクセストークン更新に失敗しました。\n\n`
      + `エラー内容: ${errorMessage}\n\n`
